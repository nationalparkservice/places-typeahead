...
$(document).ready(function () {
  ...
  var types = []
  var url = 'https://nps.cartodb.com/api/v2/sql?q=SELECT points_of_interest.places_id,points_of_interest.name,points_of_interest.places_id,parks.full_name,parks.unit_code,st_y(points_of_interest.the_geom) as lat,st_x(points_of_interest.the_geom) as lng,points_of_interest.type FROM points_of_interest,parks {{where}} ORDER BY points_of_interest.name,parks.full_name LIMIT(10)' // Change limit to five and no results display.
  var where = baseWhere
  var map
  var marker
  ...
  function createTypeahead () {
    ...
    $typeahead
      .typeahead({
        ...
        templates: {
          footer: '<div class="add-to-places">Add this place?</div>',
          // pending: 'Pending',
